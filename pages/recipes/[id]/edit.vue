      ...
      <form v-else-if="form.title !== undefined" @submit.prevent="submit" class="space-y-6">
            ...
            <div>
              ...
              <div class="flex items-center p-1 bg-slate-100 rounded-xl border border-slate-200 w-fit">
                <button type="button" @click="form.salt = true" class="px-3 sm:px-4 py-1.5 text-xs sm:text-sm rounded-lg transition-all"
                  :class="form.salt ? 'bg-white font-bold text-sage-300 shadow-sm' : 'font-medium text-slate-500'">🧂 Salé</button>
                <button type="button" @click="form.salt = false" class="px-3 sm:px-4 py-1.5 text-xs sm:text-sm rounded-lg transition-all"
                  :class="!form.salt ? 'bg-white font-bold text-sage-600 shadow-sm' : 'font-medium text-slate-500'">🍰 Sucré</button>
              </div>
            </div>
        ...
        <div class="rounded-2xl bg-slate-50 p-5 ring-1 ring-slate-100">
          <div class="flex items-center justify-between mb-4">
            ...
            <button type="button" @click="addIngredient" class="text-xs font-bold text-sage-300 hover:bg-sage-50 px-3 py-1.5 rounded-lg">+ Ajouter</button>
          </div>
          <div class="space-y-3">
            <div v-for="(ing, i) in form.ingredients" :key="i" class="grid grid-cols-[1fr_auto_auto_auto] gap-2 items-start">
              <input v-model="ing.item" type="text" placeholder="Ingrédient" required class="w-full rounded-lg border-slate-200 bg-white px-3 py-2 text-sm" />
              <input v-model.number="ing.quantity" type="number" step="any" placeholder="Qté" class="w-20 rounded-lg border-slate-200 bg-white px-3 py-2 text-sm" />
              <input v-model="ing.unit" type="text" placeholder="Unité" class="w-20 rounded-lg border-slate-200 bg-white px-3 py-2 text-sm" />
        ...
        <div class="rounded-2xl bg-slate-50 p-5 ring-1 ring-slate-100">
          <div class="flex items-center justify-between mb-4">
            <label class="text-sm font-bold text-slate-900">🍳 Étapes</label>
            <button type="button" @click="addStep" class="text-xs font-bold text-sage-300 hover:bg-sage-50 px-3 py-1.5 rounded-lg">+ Ajouter</button>
          </div>
          <div class="space-y-3">
            <div v-for="(step, i) in form.steps" :key="i" class="flex items-start gap-3 bg-white rounded-xl border border-slate-200 p-3 shadow-sm">
              ...
              <textarea v-model="form.steps[i]" placeholder="Décrivez l'étape..." rows="3"
                class="flex-1 rounded-lg border-slate-200 bg-slate-50 px-3 py-2 text-sm focus:bg-white resize-y min-h-[80px]" />
        ...
        <div class="flex items-center justify-between pt-4 border-t border-slate-100">
          <button type="button" @click="deleteRecipe"
            class="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold text-rose-600 hover:bg-rose-50 transition-colors">
          ...
          <div class="flex items-center gap-3">
            <NuxtLink :to="`/recipes/${id}`" class="rounded-xl px-5 py-2.5 text-sm font-bold text-slate-600 hover:bg-slate-100">Annuler</NuxtLink>
