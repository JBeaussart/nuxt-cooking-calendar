    ...
    <div class="w-full max-w-md">
      <div class="bg-white dark:bg-stone-800 rounded-2xl shadow-xl p-8">
        ...
        <form @submit.prevent="handleSignup" class="space-y-6">
          ...
          <div>
            <label class="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-2">Confirmer le mot de passe</label>
            <input
              v-model="confirm"
              type="password"
              required
          ...
          <button
            type="submit"
            :disabled="loading"
            class="w-full bg-gradient-to-r from-saffron-300 to-saffron-500 text-white py-3 rounded-lg font-semibold hover:from-saffron-300 hover:to-saffron-600 transition shadow-lg disabled:opacity-60"
          >
        ...
        <div class="mt-6 text-center">
          <p class="text-stone-600 dark:text-stone-400">
            Déjà un compte ?
            <NuxtLink to="/login" class="text-saffron-600 hover:text-saffron-700 dark:text-saffron-400 dark:hover:text-saffron-300 font-medium ml-1">Se connecter</NuxtLink>
          </p>
        </div>
      </div>
    </div>
